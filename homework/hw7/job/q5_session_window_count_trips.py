"""
This script sets up a Flink streaming job that reads from a Kafka topic, applies session windowing, and writes the results to a PostgreSQL database.
It includes robust logging and error handling to ensure smooth execution and easier debugging.
"""
...
def ensure_postgres_table():
    logger.info("Ensuring Postgres table 'session_window_counts' exists...")
    ...
    for i in range(10):
        try:
            ...
            logger.info("Successfully verified/created Postgres table - session_window_counts.")
            return
...
def run_session_window_job():
    logger.info("Starting Flink job to process session windows...")
    # 1. Environment Setup
    ...
    logger.info("Creating session_sink table in Postgres if it doesn't exist...")
    t_env.execute_sql("""
    ...
    logger.info("Inserting into session_sink using SESSION windows...")
...
if __name__ == '__main__':
    ...
    run_session_window_job() # Start Flink job to process data and write to Postgres